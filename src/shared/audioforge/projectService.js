export class AudioForgeProjectService {
  constructor() {
  ...
  async prepareProjectForSave(projectPath, timelineStore, mediaStore) {
    // Extract project name from path (remove .afproj extension)
    const projectName = projectPath.split('/').pop().replace('.afproj', '');
  ...
  async loadProject(projectPath, projectData, timelineStore, mediaStore) {
    // Validate app type
    if (projectData.appType && projectData.appType !== 'audioforge') {
      throw new Error(`This project was created in ${projectData.appType} and cannot be opened in AudioForge`);
    }
    
    // Extract project folder and name from path
    const pathParts = projectPath.split('/');
    const projectName = pathParts.pop().replace('.afproj', '');
